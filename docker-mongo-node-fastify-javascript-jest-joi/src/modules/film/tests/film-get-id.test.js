...
describe("Endpoint to get all films", () => {
  ...
  beforeAll(async () => {
    const axiosResponse = await axios.get(
      `http://localhost:3000/film/${response.body.data.films[1]._id}`
    );
  ...
  it("Film should be have title, episode_id, director, producer and release_date", () => {
    const film = response.body.data.film;
